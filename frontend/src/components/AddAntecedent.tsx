...
import { CREATE_EVENT_MUTATION } from '@/graphql/queries/create-event.ts'

export const AddAntecedent = ({ eventId }: { eventId: number }) => {
  ...
  useEffect(() => {
    if (resetForm) {
      setFieldValue('antecedent', eventId.toString() ?? '')
  ...
  return (
    <DialogTrigger>
      <Button
        ...
        ghost={true}
        size={'sm'}
        className={'rounded-t-none border border-gray-200'}
